...
const Playlists: React.FC<Props> = ({ item }) => {
    return (
        <Link href={`/playlists/${item?.id}`} style={{ width: '17.6%', textDecoration: 'none', color: 'white' }}>
            <Stack sx={[{ width: '100%', minHeight: '300px', height: '100%', position: "relative", borderRadius: '10px', overflow: 'hidden', background: "rgba(255, 255, 255, 0.1);", cursor: 'pointer', transition: ".3s ease", padding: "13px", gap: "10px" }, { '&:hover': { background: "rgba(255, 255, 255, 0.2);" } }]}>
