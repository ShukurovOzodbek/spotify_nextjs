...
const Aritsts: React.FC<IProps> = ({ item }) => {
    ...
    return (
        <Link onClick={handleScroll} href={`/artists/${item.id}`} style={{ width: '17.6%', textDecoration: 'none', color: 'white' }}>
            <Stack sx={[{ display: 'flex', flexDirection: 'column', color: 'white', textDecoration: 'none', width: '100%', height: '250px', position: "relative", borderRadius: '10px', overflow: 'hidden', background: "rgba(255, 255, 255, 0.1);", cursor: 'pointer', transition: ".3s ease", padding: "13px", justifyContent: 'space-between'}, { '&:hover': { background: "rgba(255, 255, 255, 0.2);" } }]}>
