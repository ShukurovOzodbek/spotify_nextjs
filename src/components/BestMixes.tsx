...
const BestMixes: React.FC<Props> = ({ item }) => {
    return (
        <Link href={`/albums/${item.id}`} style={{ width: '18%', textDecoration: 'none', color: 'white' }}>
            <Stack sx={[{ width: '100%', height: '300px', position: "relative", borderRadius: '10px', overflow: 'hidden', background: "rgba(255, 255, 255, 0.1);", cursor: 'pointer', transition: ".3s ease", padding: "20px", gap: "10px" }, { '&:hover': { background: "rgba(255, 255, 255, 0.2);" } }]}>
                <img src={item.images[1].url} style={{ width: "100%", borderRadius: '10px' }} alt="" />
                <Stack sx={{ gap: "10px" }}>
