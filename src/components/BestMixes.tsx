...
const BestMixes: React.FC<Props> = ({ item }) => {
    return (
        <Link href={`/albums/${item.id}`} onClick={() => handleSubmit(item.name)} style={{ width: '17.6%', textDecoration: 'none', color: 'white' }}>
            <Stack className='css-box-model' sx={[{ width: '100%', height: '100%', position: "relative", borderRadius: '10px', overflow: 'hidden', background: "rgba(255, 255, 255, 0.1);", cursor: 'pointer', transition: ".3s ease", padding: "13px", gap: "10px" }, { '&:hover': { background: "rgba(255, 255, 255, 0.2);" } }]}>
